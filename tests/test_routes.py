...
def test_update_book_invalid(client, two_saved_books):
    #Arrange
    test_data = {
        "title": "New Book",
        "description": "The Best!"
...
def delete_book(client, two_saved_books):
    #Act
...
def test_validate_model_missing_record(client, two_saved_books):
    #Act/Assert
    #Calling 'validate_model'  without being invoked by a route will cause
    # an 'HTTPException' when an 'abort' stmt is reached
    with pytest.raises(HTTPException):
        result_book = validate_model(Book, "books/3")

def test_validate_model_invalid_id(client, two_saved_books):
    #Act/Assert
    #Calling 'validate_model'  without being invoked by a route will cause
    # an 'HTTPException' when an 'abort' stmt is reached
    with pytest.raises(HTTPException):
        result_book = validate_model(Book,"books/cat")
